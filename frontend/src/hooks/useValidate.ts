...
const useValidate = (FormData: any) => {
    ...
    const validate = ({target:{name,value}}:any,single:boolean=true,formLength:number=1) => {
        let i = 0
            ...
            if(name === Name){
                ...
                if(Value.required && value === ''){
                    ErrorSyncState.setValue({
                        ...
                        [name]:{
                            ...(ErrorSyncState as any).value[name],
                            required:false
                        }
                    })
                }
                ...
                if(Value.minMaxWords && (value.split(' ').length < Value.minMaxWords.value[0] || value.split(' ').length > Value.minMaxWords.value[1])){
                    ErrorSyncState.setValue({
                        ...ErrorSyncState.value,
                        [name]:{
                            ...(ErrorSyncState as any).value[name],
    ...
    const isItValid = () => {
        ...
        Object.values(Errors).some(value => {
            const Error = value as FormDataInterface
            if(
                Error.required &&
                Error.min?.state &&
                Error.max?.state &&
